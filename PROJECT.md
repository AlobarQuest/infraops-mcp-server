...
- [ ] (P1) cloudflare_list_pages_projects redaction is incomplete: it masks DROPBOX_APP_SECRET, SUPABASE_SERVICE_ROLE_KEY and VITE_SUPABASE_ANON_KEY but returns GEMINI_API_KEY in plaintext with reveal=false. Redaction should be allowlist-based (mask every deployment_configs/env_vars value unless reveal=true), not keyed on name patterns. — added 2026-08-02
- [ ] (P1) The 04:00 change-window executor uses a DENYLIST (source !== 'security') to decide what it hands to an LLM agent holding production Coolify tools, so every NEW change-manager source is admitted by default. Proven by rotation, which the executor already admits and which nobody designed it to. Make it an allowlist of the derived pipelines it is meant to execute. SEQUENCING CONSTRAINT WITHDRAWN 2026-08-10 — it was too strong, and the correction matters. Devon approved item 44 the same day, putting it in the exact state the constraint feared, and BOTH change-manager-side guards held when tested against that real state rather than a hypothetical one: the executor's own query GET /api/items?status=approved returned 0 items (proposed sources are withheld when no source is named), and POST /api/items/44/claim returned 409 "'deploy' changes have no authorized executor". Two independent guards, in the service that owns the data, so approving a deploy record creates no exposure. Fix the denylist anyway: a consumer that admits every unknown producer by default is wrong on its own terms, and rotation already proves it happens silently. This is defence in depth, not a race. Found by the ADR-0019 Increment 1 adversarial review, 2026-08-10. — added 2026-08-10
## Future plans
