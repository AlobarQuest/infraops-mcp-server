...
function auditResult(
  proposals: Proposal[],
  source: AuditResult['meta']['standards_source'] = 'live',
  errors?: string[],
): AuditResult {
  return {
    meta: {
...
describe('buildDriftReport', () => {
  it('assembles ok sections and computes totals + delta', async () => {
    ...
    const auditFn = async (inst: string) =>
      inst === 'prod' ? auditResult([p1, p2]) : auditResult([]);
...
describe('wasCleanlyAudited', () => {
  ...
  it('is false when every instance errored out (e.g. missing tokens)', async () => {
    const auditFn = async () =>
      auditResult([], 'cache', ['applications: TOKEN required', 'databases: TOKEN required']);
