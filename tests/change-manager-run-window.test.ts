...
function item(id: number): ApprovedItem {
  return { id, identity: `prod::coolify.force_https::u${id}`, instance: "prod", rule_key: "coolify.force_https",
...
describe("runWindow", () => {
  ...
  it("a postOutcome failure does not abort the batch", async () => {
    const d = deps({
      getApproved: vi.fn(async () => [item(1), item(2)]),
      postOutcome: vi.fn(async (id: number) => { if (id === 1) throw new Error("network"); }),
    });
    const summary = await runWindow(d);
  ...
  it("surfaces a getApproved failure to the caller (abort cleanly, nothing applied)", async () => {
    const d = deps({ getApproved: vi.fn(async () => { throw new Error("unreachable"); }) });
