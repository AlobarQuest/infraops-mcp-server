...
import { buildEscalations, toEscalation, type ClassifiedFinding } from "../src/security-drift/emit.js";
...
describe("toEscalation", () => {
  it("maps a URGENT shell secret to a manual question escalation", () => {
    const e = toEscalation(cf("shell.plaintext_secret", "/Users/x/.zshrc", "/Users/x/.zshrc: TOKEN=<inline value>"));
  ...
  it("maps a NORMAL os toggle to a remediation with an exact exec", () => {
    const e = toEscalation(cf("os.screen_lock", "screen lock off", "screen lock off (askForPassword=0)"));
...
describe("buildEscalations", () => {
  it("records a plan-hash keyed by fingerprint that matches planHash(plan)", () => {
    const item = cf("shell.plaintext_secret", "/Users/x/.zshrc", "/Users/x/.zshrc: TOKEN=<inline value>");
