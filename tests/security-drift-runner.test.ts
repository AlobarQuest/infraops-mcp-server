...
let dir: string;
beforeEach(() => { dir = fs.mkdtempSync(path.join(os.tmpdir(), "sd-runner-")); });
...
function deps() {
  ...
  return {
    posted, emailed,
    d: {
      postSync: async (b: SyncBody) => { posted.push(b); return { new: 0, refreshed: 0, resolved: 0, reopened: 0 }; },
      sendUrgent: async (items: any[]) => { emailed.push(items.length); return true; },
    },
  };
}
