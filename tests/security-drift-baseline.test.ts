...
} from '../src/security-drift/baseline.js';
import type { Finding } from '../src/security-drift/scan-parser.js';
import type { Classification } from '../src/security-drift/taxonomy.js';

let dir: string;
beforeEach(() => {
  dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sd-baseline-'));
});
afterEach(() => {
  fs.rmSync(dir, { recursive: true, force: true });
});

const finding = (check: string, target: string): Finding => ({
  severity: 'FAIL',
  check,
  detail: target,
  target,
});
const cls = (tier: Classification['tier']): Classification => ({
  tier,
  kind: 'question',
  risk: 'caution',
  remediation: { manual: ['x'] },
  title: 't',
});
...
describe('diffFindings', () => {
  it('flags new findings and tier changes, suppresses unchanged', () => {
    const a = {
      finding: finding('credfile.over_permissive', '/a'),
      classification: cls('AUTO_FIX'),
    };
