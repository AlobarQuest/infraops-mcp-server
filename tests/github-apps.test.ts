...
describe('github app tools', () => {
  ...
  it('list returns a compact summary by default', async () => {
    ...
    const res = await mockServer._handlers['coolify_list_github_apps']({
      summary: true,
      instance: 'prod',
    });
  ...
  it('list_repos unwraps { repositories: [] }', async () => {
    ...
    const res = await mockServer._handlers['coolify_list_github_app_repos']({
      id: 1,
      instance: 'prod',
    });
  ...
  it('list_branches encodes owner/repo in the path', async () => {
    ...
    expect(client.coolifyGet).toHaveBeenCalledWith(
      '/github-apps/1/repositories/a%2Fb/c%20d/branches',
      undefined,
      'prod',
    );
  });
  ...
  it('create requires an explicit instance', () => {
    expect(
      mockServer._schemas['coolify_create_github_app'].inputSchema.instance.safeParse(undefined)
        .success,
    ).toBe(false);
  });
});
