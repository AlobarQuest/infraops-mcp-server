...
beforeEach(() => {
  dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sd-exec-'));
...
function item(over: Partial<ApprovedItem> = {}): ApprovedItem {
  return {
    id: 1,
    identity: 'mac::sec.os.screen_lock::fp1',
    instance: 'mac',
    rule_key: 'sec.os.screen_lock',
    resource_type: 'os',
    resource_uuid: 'fp1',
    resource_name: 'Screen lock',
    risk: 'safe',
    kind: 'remediation',
    reasoning: '[NORMAL] os.screen_lock',
    note: null,
    status: 'approved',
    source: 'security',
...
function deps(items: ApprovedItem[], exec?: (cmd: string[]) => ExecResult) {
  ...
  return {
    ...
    d: {
      ...
      onIntegrityFailure: async (it: ApprovedItem, reason: string) => {
...
describe('runSecurityWindow', () => {
  ...
  it('REFUSES to run when the plan hash does not match (tamper) and never execs', async () => {
    const it = item();
    saveEmitState(emitStateFile, {
      fp1: { hash: 'deadbeef-not-the-real-hash', ts: '2026-06-15T03:00:00Z' },
    });
    const spy = vi.fn((_cmd: string[]): ExecResult => ({ ok: true, detail: 'ok' }));
  ...
  it('tracks (does not execute) a manual remediation even with a valid hash', async () => {
    const it = item({
      plan: { tier: 'URGENT', source: 'security', remediation: { manual: ['rotate the token'] } },
    });
    saveEmitState(emitStateFile, { fp1: { hash: planHash(it.plan), ts: '2026-06-15T03:00:00Z' } });
    const spy = vi.fn((_cmd: string[]): ExecResult => ({ ok: true, detail: 'ok' }));
...
describe('run-window excludes security items', () => {
  it('never hands a source=security item to the Coolify agent', async () => {
    ...
    const s = await runWindow({
      ...
      runAgent: async (it) => {
        ran.push(it.id);
        return { outcome: 'done', detail: '', rollback: {}, tool_calls: { calls: [] } };
      },
      postOutcome: async () => {},
