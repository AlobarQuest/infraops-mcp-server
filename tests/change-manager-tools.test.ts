...
describe("curated tools", () => {
  ...
  it("set_application_domains rejects a non-https domain in the list", async () => {
    const ctx = { instance: "prod" as const, rollback: {} };
    await expect(runTool("set_application_domains", { uuid: "u1", domains: "https://x.com,http://y.com" }, ctx))
  ...
  it("set_application_healthcheck rejects a bad path or non-integer port", async () => {
    const ctx = { instance: "prod" as const, rollback: {} };
    await expect(runTool("set_application_healthcheck", { uuid: "u1", path: "health", port: 3000 }, ctx)).rejects.toThrow();
    await expect(runTool("set_application_healthcheck", { uuid: "u1", path: "/health", port: 3.5 }, ctx)).rejects.toThrow();
    expect(coolifyPatch).not.toHaveBeenCalled();
  });

  it("a control tool passed to runTool throws (handled by the agent loop, not runTool)", async () => {
    await expect(runTool("report_done", { summary: "x" }, { instance: "prod", rollback: {} }))
