...
describe('classify', () => {
  ...
  it('URGENTs a re-inlined shell secret (never auto)', () => {
    const c = classify(
      f({
        check: 'shell.plaintext_secret',
        target: '/Users/x/.zshrc',
        detail: '/Users/x/.zshrc: TOKEN=<inline value>',
      }),
      { autoFixAllowlist: [] },
    );
    expect(c?.tier).toBe('URGENT');
  ...
  it('classifies an OS hardening toggle as NORMAL with an exact exec', () => {
    const c = classify(
      f({
        check: 'os.screen_lock',
        target: 'screen lock off (askForPassword=0)',
        detail: 'screen lock off (askForPassword=0)',
      }),
      { autoFixAllowlist: [] },
    );
  ...
  it('URGENTs an unknown check key (deny-by-default)', () => {
    const c = classify(f({ check: 'brand.new_thing', target: 'x', detail: 'x' }), {
      autoFixAllowlist: [],
    });
    expect(c?.tier).toBe('URGENT');
  });
  ...
  it('drops a false-positive .env.example', () => {
    const c = classify(
      f({ target: '/Users/x/.env.example', detail: '/Users/x/.env.example (mode 644)' }),
      { autoFixAllowlist: ['/Users/x/.env.example'] },
    );
    expect(c).toBeNull();
  });
  ...
  it('drops a public fullchain.pem false positive', () => {
    const c = classify(
      f({
        check: 'credfile.private_key',
        target: '/Users/x/certs/fullchain.pem',
        detail: '/Users/x/certs/fullchain.pem (mode 644)',
      }),
      { autoFixAllowlist: [] },
    );
    expect(c).toBeNull();
  ...
  it('escalates controlplane.drift even when the changed path contains an FP token like /test/', () => {
    const detail =
      'uncommitted/untracked control-plane change:  ?? hooks/test/evil.sh (review + commit if intended)';
    const c = classify(f({ check: 'controlplane.drift', target: detail, detail }), {
      autoFixAllowlist: [],
    });
    expect(c?.tier).toBe('URGENT');
  });
