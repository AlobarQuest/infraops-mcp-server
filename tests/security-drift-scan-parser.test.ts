...
describe('parseScan', () => {
  ...
  it('keeps only the key name for secret-bearing details (no value reconstruction)', () => {
    const out =
      'FAIL shell.plaintext_secret            /Users/x/.zshrc: OPENAI_API_KEY=<inline value> (move to Keychain)';
    const f = parseScan(out);
...
describe('extractTarget', () => {
  it("splits on ' (mode ' even when the path has spaces", () => {
    expect(extractTarget('/Users/devon/from iMac/downloads/k.pem (mode 022) world-writable')).toBe(
      '/Users/devon/from iMac/downloads/k.pem',
    );
  });
  it("splits a 'file: rest' detail at the colon", () => {
    expect(extractTarget('claude_desktop_config.json: SOME_TOKEN has inline value')).toBe(
      'claude_desktop_config.json',
    );
  });
  it('falls back to the whole detail for listeners/os checks', () => {
    expect(extractTarget('port 6379 bound to 0.0.0.0 (want 127.0.0.1)')).toBe(
      'port 6379 bound to 0.0.0.0 (want 127.0.0.1)',
    );
  });
});
