...
Ship `coolify_audit_standards` in infraops: a stateless, read-only tool that fetches machine-readable standards from infra-brain over a new REST API and emits executable remediation proposals for live Coolify resources. infra-brain owns *what* the standards are; infraops owns *how* to observe and remediate.
