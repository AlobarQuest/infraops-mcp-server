...
>    form (the same one `coolify_deploy` uses). _(Corrected after the first live run on
>    2026-06-14: the initial `POST /applications/{uuid}/deploy` form — copied from
>    `reset_labels` — returns 404; see BACKLOG #5.)_
...
>    _captured_ rollback. The executor now: (a) **pre-validates** live before the agent
...
function item(): ApprovedItem {
  return {
    ...
    risk: 'caution',
    kind: 'remediation',
    reasoning: 'must be https',
    plan: { steps: ['set https', 'redeploy'] },
    note: null,
    status: 'in_progress',
  };
}
...
const toolTurn = (name: string, input: any) => ({
...
describe('runChangeAgent', () => {
  it('runs tool calls then report_done → outcome done (post-verify passes) with tool_calls recorded', async () => {
    ...
    coolifyPatch.mockImplementation(async (_p: string, body: any) => {
 ...
 */
async function postVerifyOrRevert(
  item: ApprovedItem,
  ctx: ToolCtx,
  calls: ToolCalls,
  ...
  try {
    if (ctx.rollback.domains !== undefined) {
      ...
      if (!app || !httpsConformant(app)) {
        ...
        return {
          outcome: 'failed',
          detail: 'post-verify failed: domains not https after change; reverted via rollback',
    ...
    if (ctx.rollback.health_check_enabled !== undefined) {
      ...
      if (!app || app.health_check_enabled !== true) {
        ...
        return {
          outcome: 'failed',
          detail:
            'post-verify failed: health check not enabled after change; reverted via rollback',
          rollback: ctx.rollback,
          tool_calls: { calls },
        };
      }
...
export function renderWindowMarkdown(generatedAt: string, s: WindowSummary): string {
  ...
  lines.push('');
  lines.push(
    `**${s.applied} applied**, ${s.blocked} blocked, ${s.failed} failed, ${s.skipped} skipped (of ${s.considered} considered).`,
  );
  lines.push('');
  if (!s.results.length) {
...
async function doSync(reportDir: string, now: string): Promise<void> {
  ...
  const summary = await client().postSync({
    generated_at: now,
    source_report: `${date}.remediation.json`,
...
async function doRunWindow(reportDir: string | undefined, now: string): Promise<void> {
  ...
  const summary = await runWindow({
    ...
    postOutcome: async (id, body) => {
      await c.postOutcome(id, body);
    },
    maxChangesPerWindow: Number.parseInt(process.env.MAX_CHANGES_PER_WINDOW ?? '5', 10) || 5,
  });
  await c.finishWindow(wr.id, {
    status: 'done',
