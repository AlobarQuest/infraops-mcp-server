...
Cheap, but it is still a control-plane URGENT raised by a *legitimate* action. That trains the
...
`deployed == blessed source` *is* the drift check.
...
- Extend `SelfCheckConfig`:
  ```ts
  sourceVerifiedFiles: { deployed: string; source: string }[];
