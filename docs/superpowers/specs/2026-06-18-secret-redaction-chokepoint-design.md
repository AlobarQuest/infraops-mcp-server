...
with an audited `reveal` opt-out and an always-bypass list for tools whose output *is* the
...
2. **Value-shape** (redact the value regardless of field name):
   ...
   - **Guard:** SSH *public* material (`ssh-ed25519 AAAA…`, `ssh-rsa AAAA…`) is NOT redacted —
...
2. **Per-call `reveal: true`:** the wrapper reads `args.reveal`; if `true`, return the result
   ...
   `supabase_get_api_keys` *behind* `reveal` (today ungated).
