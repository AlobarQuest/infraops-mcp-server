...
export interface RotationDeps {
  ...
  probe(kind: ProviderProbe, value: string, opts?: { email?: string; workspace?: string }): Promise<number>;
...
export async function runRotationPlan(plan: RotationPlanSpec, deps: RotationDeps): Promise<RotationOutcome> {
  ...
  try {
    ...
    if (!policy || !policy.executor) return finish("blocked", `class ${plan.credClass} is not executor-runnable`);
    if (!plan.consumersVerified) return finish("blocked", "consumer set not attested — refusing (fail-safe)");
    ...
    if (plan.keeperBwsUuid) {
      ...
      if (oldValue === null) return finish("failed", `old-value secret ${plan.retireBwsUuids[0]} unreadable`);
    }
    ...
    if (oldValue === null) return finish("blocked", "old value unavailable — confirm the provider revoke manually, then resolve-exposure");
...
async function deployConsumer(
  ...
  switch (consumer.kind) {
    ...
    case "keychain": {
      if (!consumer.service || !consumer.account) throw new Error("keychain consumer missing service/account");
    ...
    case "gh-actions-secret": {
      if (!consumer.repo || !consumer.name) throw new Error("gh-actions-secret consumer missing repo/name");
  ...
  return {
    ...
    coolify: {
      async getEnv(instance, resourceType, uuid, key) {
        const envs = await io.coolifyGet<Array<{ key: string; value?: string; real_value?: string }>>(
          envPath(resourceType, uuid),
          instance,
        );
