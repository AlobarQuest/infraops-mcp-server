...
//   - ORDER: verify-new → store → deploy → revoke-confirm. Each phase early-returns on
...
export interface RotationDeps {
  ...
  probe(
    kind: ProviderProbe,
    value: string,
    opts?: { email?: string; workspace?: string },
  ): Promise<number>;
...
class Scrubber {
  ...
  add<T extends string | null>(v: T): T {
    if (v && v.length >= 8) this.values.push(v);
    return v;
  }
  clean(detail: string): string {
    let out = detail;
...
export async function runRotationPlan(
  plan: RotationPlanSpec,
  deps: RotationDeps,
): Promise<RotationOutcome> {
  const scrub = new Scrubber();
  const steps: string[] = [];
  const finish = (outcome: RotationOutcome['outcome'], msg: string): RotationOutcome => ({
    outcome,
  ...
  try {
    ...
    if (plan.keeperBwsUuid) {
      ...
      const staged = scrub.add(
        await deps.keychain.read(plan.staging.service, plan.staging.account),
      );
      ...
      const status = await deps.probe(plan.providerProbe, newValue, {
        email: plan.probeEmail,
        workspace: plan.probeWorkspace,
      });
      if (status !== 200) {
        return finish(
          'failed',
          `verify FAILED: new credential probe returned ${status} (expected 200) — check the staged value; nothing was written or revoked`,
        );
      }
      ...
      if (staged) {
        if (!quarantine) {
          if (keeperCurrent === staged) {
            ...
            return finish(
              'blocked',
              'keeper already updated but no quarantined old value exists — confirm the provider revoke manually, then `security-drift-cli resolve-exposure`',
            );
          }
          quarantineUuid = await deps.bws.create(
            plan.quarantineName,
            keeperCurrent,
            plan.bwsProjectId,
          );
      ...
      oldValue = quarantine
        ? quarantine.value
        : scrub.add(quarantineUuid ? await deps.bws.getValue(quarantineUuid) : null);
      ...
      if (!plan.retireBwsUuids.length)
        return finish(
          'blocked',
          'no old-value source (retireBwsUuids empty) — cannot confirm revoke',
        );
    ...
    if (plan.providerProbe === 'github' && !(await deps.ghKeeperOk())) {
      return finish(
        'failed',
        'gh keeper does NOT authenticate — investigate before any revoke bookkeeping',
      );
    }
    ...
    if (oldValue === null)
      return finish(
        'blocked',
        'old value unavailable — confirm the provider revoke manually, then resolve-exposure',
      );
    ...
    if (oldStatus === 200) {
      return finish(
        'blocked',
        `old credential still LIVE at the provider — complete the console revoke, then re-approve. Steps: ${plan.manualSteps.join(' → ')}`,
      );
    }
    if (oldStatus !== DEAD_STATUS) {
      return finish(
        'blocked',
        `old-credential probe indeterminate (${oldStatus}) — refusing to retire; re-approve to retry`,
      );
    }
    ...
    if (plan.providerProbe === 'github' && !(await deps.ghKeeperOk())) {
      return finish(
        'failed',
        'old credential dead but gh keeper NO LONGER authenticates — the wrong token may have been revoked; investigate NOW',
      );
    }
    ...
    return finish(
      'done',
      `rotation complete for ${plan.credId} — exposure(s) ${plan.exposureIds.join(', ') || 'n/a'} closed`,
    );
  } catch (e) {
...
async function deployConsumer(
  ...
  switch (consumer.kind) {
    case 'bws-secret': {
      if (consumer.uuid === plan.keeperBwsUuid) return null; // the keeper itself (store phase)
      if (!consumer.uuid) throw new Error('bws-secret consumer missing uuid');
    ...
    case 'coolify-env': {
      ...
      const current = scrub.add(
        await deps.coolify.getEnv(
          consumer.instance,
...
const BEARER_PROBE_URLS: Record<'github' | 'openrouter' | 'openai', string> = {
  ...
  return {
    ...
    coolify: {
      async getEnv(instance, resourceType, uuid, key) {
        const envs = await io.coolifyGet<
          Array<{ key: string; value?: string; real_value?: string }>
        >(envPath(resourceType, uuid), instance);
    ...
    async probe(kind, value, opts) {
      ...
      if (kind === 'bitbucket') {
        ...
        if (!opts?.email || !opts?.workspace) return 400; // misconfigured plan → not "dead"
