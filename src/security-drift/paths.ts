...
export function securityPaths(): SecurityPaths {
  ...
  return {
    ...
    stateDir,
    scanPath:
      process.env.SECURITY_SCAN_PATH ?? path.join(home, '.claude', 'bin', 'security-scan.sh'),
    scanSourcePath:
      process.env.SECURITY_SCAN_SOURCE_PATH ??
    ...
    auditLog:
      ...
      path.join(home, '.claude', 'audit', 'high-power-actions.jsonl'),
