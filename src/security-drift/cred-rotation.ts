...
export function loadRotationState(file: string): RotationState {
  const parsed = loadValidated0600Json<Partial<RotationState>>(
    file,
    'rotation-state',
 ...
 */
export function buildCredClassifications(
  specs: CredentialSpec[],
  state: RotationState,
...
function rotationClassification(spec: CredentialSpec, state: RotationState): Classification {
  ...
  if (!spec.consumers_verified)
    blockers.push(
      'consumer set not attested (consumers_verified missing) — FAIL-SAFE: never revoke an unmapped credential',
    );
  ...
  if (!spec.bws_uuid)
    blockers.push(
      'no BWS copy of the old value — executor cannot confirm provider revoke (401 probe)',
    );
