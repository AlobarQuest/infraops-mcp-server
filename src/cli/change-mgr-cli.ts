...
function credRotationBwsToken(): string | undefined {
  if (process.env.BWS_CRED_ROTATION_TOKEN) return process.env.BWS_CRED_ROTATION_TOKEN;
  try {
    const v = execFileSync(
      '/usr/bin/security',
      ['find-generic-password', '-s', 'Claude', '-a', 'BWS_ACCESS_TOKEN_CRED_ROTATION', '-w'],
...
async function doRunSecurityWindow(reportDir: string | undefined, now: string): Promise<void> {
  ...
  const summary = await runSecurityWindow({
    ...
    onIntegrityFailure: async (item, reason) => {
      await sendAlertEmail(
        '🚨 Security executor REFUSED a plan (integrity)',
        `${item.resource_name} (item ${item.id}): ${reason}`,
        notifyDeps,
      );
    },
    ...
    rotation: rotationToken
      ? defaultRotationDeps({
          coolifyGet: (pth, instance) =>
            coolifyGet(pth, undefined, (instance ?? 'prod') as 'prod' | 'dev'),
          coolifyPatch: (pth, body, instance) =>
            coolifyPatch(
              pth,
              body as Record<string, unknown>,
              (instance ?? 'prod') as 'prod' | 'dev',
            ),
  ...
  if (!rotationToken) {
    process.stdout.write(
      'note: BWS_ACCESS_TOKEN_CRED_ROTATION not configured — executor-assisted rotations will BLOCK (no broad-token fallback).\n',
    );
  }
  const lines = [
    `# Security window — ${now}`,
    ...
    ...summary.results.map(
      (r) =>
        `- ${r.outcome === 'done' ? '✅' : r.outcome === 'blocked' ? '🚫' : r.outcome === 'failed' ? '❌' : '⏭️'} **${r.name}** — ${r.outcome}: ${r.detail}`,
    ),
  ];
  const md = lines.join('\n');
  if (reportDir)
...
async function main(): Promise<void> {
  ...
  if (args.command === 'sync') {
    ...
    throw new Error(
      `unknown command: ${String(args.command)} (use sync | run-window | run-security-window)`,
    );
  }
}
