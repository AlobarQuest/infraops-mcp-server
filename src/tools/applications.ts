...
export function registerApplicationTools(server: McpServer): void {
  ...
  server.registerTool(
    'coolify_list_applications',
    {
      ...
      description:
        'List all applications across the Coolify instance. Returns a compact summary ' +
    ...
    async ({
      ...
      try {
        ...
        const out = summary
          ? summarize(apps as any[], toApplicationSummary, true)
          : maskSensitiveList(apps as any[], reveal);
  ...
  server.registerTool(
    ...
    async ({
      ...
      try {
        ...
        return jsonResponse(maskSensitive(app as any, reveal));
      } catch (error) {
        return {
          isError: true,
          content: [{ type: 'text', text: handleCoolifyError(error) }],
    ...
    'coolify_create_application_deploykey',
    {
      ...
      inputSchema: {
        ...
        git_repository: z
          ...
          .min(1)
          .describe(
            "Git SSH URL (e.g. 'git@github.com:AlobarQuest/my-app.git') — must be SSH format for deploy key auth",
          ),
        git_branch: z.string().default('main').describe('Git branch to deploy (default: main)'),
        build_pack: z
          .enum(['nixpacks', 'static', 'dockerfile', 'dockercompose'])
        ...
        domains: z
          ...
          .optional()
          .describe(
            'FQDN for single-container apps (not for dockercompose — use coolify_set_compose_config instead)',
          ),
        instance: CoolifyInstanceRequiredSchema,
      },
      annotations: {
    ...
    'coolify_set_compose_config',
    {
      ...
      inputSchema: {
        ...
        reset_labels: z
          ...
          .default(true)
          .describe(
            'Clear custom_labels so Coolify auto-generates from domain config (default: true)',
          ),
        instance: CoolifyInstanceRequiredSchema,
      },
      annotations: {
  ...
  server.registerTool(
    ...
    async ({
      ...
      try {
        const logs = await coolifyGet<string>(`/applications/${uuid}/logs`, { lines }, instance);
        const text = typeof logs === 'string' ? logs : JSON.stringify(logs, null, 2);
        const safeText = redactText(text);
        const capped =
          safeText.length > CHARACTER_LIMIT
