...
export function registerGithubTools(server: McpServer): void {
  ...
  server.registerTool(
    ...
    async (params: { repo: string; title: string; key: string; read_only: boolean }) => {
      try {
        const deployKey = await githubPost<GitHubDeployKey>(`/repos/${params.repo}/keys`, {
          title: params.title,
          key: params.key,
  ...
  server.registerTool(
    ...
    async ({ repo }: { repo: string }) => {
      try {
        const keys = await githubGet<GitHubDeployKey[]>(`/repos/${repo}/keys`);
        return {
