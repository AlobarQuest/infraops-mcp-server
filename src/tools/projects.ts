...
export function registerProjectTools(server: McpServer): void {
  ...
  server.registerTool(
    ...
    async ({ uuid, instance }: { uuid: string; instance: CoolifyInstance }) => {
      try {
        const project = await coolifyGet<CoolifyProject>(`/projects/${uuid}`, undefined, instance);
        return {
  ...
  server.registerTool(
    ...
    async ({
      ...
      try {
        const project = await coolifyPost<CoolifyProject>(
          '/projects',
          {
            name,
  ...
  server.registerTool(
    ...
    async ({
      ...
      try {
        ...
        const project = await coolifyPatch<CoolifyProject>(`/projects/${uuid}`, data, instance);
        return {
          content: [{ type: 'text', text: JSON.stringify(project, null, 2) }],
        };
      } catch (error) {
