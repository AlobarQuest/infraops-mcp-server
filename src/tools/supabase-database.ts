...
export function registerSupabaseDatabaseTools(server: McpServer): void {
  // ── Run SQL ───────────────────────────────────────────────────────
  ...
  server.registerTool(
    "supabase_run_sql",
    {
      ...
      description:
        "Execute arbitrary SQL against a project's database. WARNING: This can modify or destroy data. Use with caution.",
      inputSchema: {
    ...
    async ({ ref, query }: { ref: string; query: string }) => {
      try {
  ...
  server.registerTool(
    "supabase_list_extensions",
    {
      ...
      description:
        "List all Postgres extensions available and their enabled status for a Supabase project database.",
      inputSchema: {
        ref: z.string().describe("Supabase project reference (e.g. 'abcdefghijklmnop')"),
      },
      annotations: {
    ...
    async ({ ref }: { ref: string }) => {
      try {
        const result = await supabaseGet<Record<string, unknown>>(
          `/projects/${ref}/database/extensions`
        );
        return {
          content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
        };
  ...
  server.registerTool(
    "supabase_get_postgres_config",
    {
      ...
      description:
        "Get the current Postgres configuration for a Supabase project, including settings like max_connections and statement_timeout.",
      inputSchema: {
        ref: z.string().describe("Supabase project reference (e.g. 'abcdefghijklmnop')"),
      },
      annotations: {
  ...
  server.registerTool(
    "supabase_update_postgres_config",
    {
      ...
      description:
        "Update Postgres configuration settings for a Supabase project. Accepts key-value pairs for settings such as max_connections and statement_timeout.",
      inputSchema: {
        ref: z.string().describe("Supabase project reference (e.g. 'abcdefghijklmnop')"),
        config: z
          .record(z.string())
    ...
    async ({ ref, config }: { ref: string; config: Record<string, string> }) => {
      try {
