...
export function registerHetznerNetworkingTools(server: McpServer): void {
  ...
  server.registerTool(
    'hetzner_update_firewall_rules',
    {
      ...
      description:
        'Replace all rules on a firewall. This is a full replacement — include all desired rules.',
      inputSchema: {
  ...
  server.registerTool(
    'hetzner_create_ssh_key',
    {
      ...
      description:
        'Register a new SSH public key in Hetzner. Can then be used when creating servers.',
      inputSchema: {
  ...
  server.registerTool(
    ...
    async ({ id, size }: { id: number; size: number }) => {
      try {
        const result = await hetznerPost<Record<string, unknown>>(`/volumes/${id}/actions/resize`, {
          size,
        });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      } catch (error) {
