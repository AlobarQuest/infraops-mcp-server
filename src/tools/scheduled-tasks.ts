...
function tasksPath(resource: string, uuid: string) {
...
export function registerScheduledTaskTools(server: McpServer): void {
  ...
  server.registerTool(
    ...
    async ({
      ...
      try {
        const body: Record<string, unknown> = { name, command, frequency, is_enabled };
  ...
  server.registerTool(
    ...
    async ({
      ...
      try {
        ...
        return {
          content: [{ type: "text", text: `Scheduled task ${task_uuid} deleted from ${resource} ${uuid}.` }],
        };
      } catch (error) {
        return { isError: true, content: [{ type: "text", text: handleCoolifyError(error) }] };
      }
  ...
  server.registerTool(
    ...
    async ({
      ...
      try {
        const data = await coolifyGet(
          `${tasksPath(resource, uuid)}/${task_uuid}/executions`,
          undefined,
          instance
        );
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
