...
export function registerEnvVarTools(server: McpServer): void {
    ...
    'coolify_create_app_env',
    {
      ...
      inputSchema: {
        ...
        is_buildtime: z
          ...
          .default(false)
          .describe(
            'Available during build phase (default: false). Use false for runtime-only vars like secrets.',
          ),
  ...
  server.registerTool(
    'coolify_bulk_set_app_env',
    {
      ...
      description:
        'Upsert ONE environment variable (key/value) across many applications at once. ' +
    ...
    async ({
      ...
      const settled = await Promise.allSettled(
        app_uuids.map((uuid) =>
          coolifyPatch(
            `/applications/${uuid}/envs`,
            { key, value, is_buildtime, is_runtime },
            instance,
          ),
        ),
      );
      const succeeded: { uuid: string }[] = [];
