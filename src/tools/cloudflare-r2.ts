...
export function registerCloudflareR2Tools(server: McpServer): void {
  // ── List R2 Buckets ──────────────────────────────────────────────
  ...
  server.registerTool(
    "cloudflare_list_r2_buckets",
    {
      ...
      description:
        "List all R2 buckets in the Cloudflare account. Returns bucket names, creation dates, and location hints.",
      inputSchema: {},
      annotations: {
        readOnlyHint: true,
  ...
  server.registerTool(
    "cloudflare_create_r2_bucket",
    {
      ...
      description:
        "Create a new R2 bucket in the Cloudflare account. Bucket names must be unique within the account.",
      inputSchema: {
        ...
        locationHint: z
          ...
          .optional()
          .describe(
            "Location hint for the bucket: wnam (Western North America), enam (Eastern North America), weur (Western Europe), eeur (Eastern Europe), apac (Asia Pacific)"
          ),
      },
      annotations: {
        readOnlyHint: false,
    ...
    async ({ name, locationHint }: { name: string; locationHint?: string }) => {
      try {
        const accountId = getAccountId();
        const body: Record<string, unknown> = { name };
        if (locationHint !== undefined) body.locationHint = locationHint;
  ...
  server.registerTool(
    "cloudflare_get_r2_bucket",
    {
      ...
      description:
        "Get details for a specific R2 bucket by name — creation date, location, and storage class.",
      inputSchema: {
  ...
  server.registerTool(
    "cloudflare_delete_r2_bucket",
    {
      ...
      description:
        "Permanently delete an R2 bucket. WARNING: This is irreversible and will delete all objects in the bucket.",
      inputSchema: {
