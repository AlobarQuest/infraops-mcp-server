...
export async function supabasePost<T>(endpoint: string, data?: unknown): Promise<T> {
  const client = getClient();
...
export async function supabaseDelete<T>(endpoint: string, data?: unknown): Promise<T> {
  const client = getClient();
