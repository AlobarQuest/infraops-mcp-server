...
export function getOrbMachine(): string {
...
export async function orbExec(
  ...
  try {
    ...
    if (e.killed && e.signal === "SIGTERM") {
      throw new Error(
        `orb run timed out after ${timeout}ms on machine '${machine}': ${command}`
      );
    }

    if (e.code === "ENOENT") {
      throw new Error(
        "orb CLI not found on PATH. Install OrbStack or ensure `orb` is available to the MCP process."
      );
    }
    ...
    throw new Error(
      `orb run failed (exit ${exitCode}) on machine '${machine}': ${stderr || e.message}`
    );
  }
}

/** Check whether the orb CLI is reachable at all. Best-effort. */
export function isOrbConfigured(): boolean {
  try {
...
export function handleOrbError(error: unknown): string {
  const msg = error instanceof Error ? error.message : String(error);
  if (msg.includes("orb CLI not found")) return `Error: ${msg}`;
