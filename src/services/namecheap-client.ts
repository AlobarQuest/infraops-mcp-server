...
let _config: NamecheapConfig | null = null;

function getClient(): { client: AxiosInstance; config: NamecheapConfig } {
...
export interface NamecheapResponse {
  Status: string;
  Errors?: { Error?: { _text: string; Number: number } | Array<{ _text: string; Number: number }> };
...
export function handleNamecheapError(error: unknown): string {
