...
export async function hetznerDelete<T>(endpoint: string): Promise<T> {
  const client = getClient();
...
export function handleHetznerError(error: unknown): string {
  if (axios.isAxiosError(error)) {
    const axErr = error as AxiosError<{ error?: { message?: string; code?: string } }>;
