...
// ── Public helpers ───────────────────────────────────────────────────
export async function supabaseGet(endpoint, params) {
    const client = getClient();
    const response = await client.get(endpoint, { params });
    return response.data;
}
export async function supabasePost(endpoint, data) {
    const client = getClient();
    const response = await client.post(endpoint, data);
    return response.data;
}
export async function supabasePut(endpoint, data) {
    const client = getClient();
    const response = await client.put(endpoint, data);
    return response.data;
}
export async function supabasePatch(endpoint, data) {
    const client = getClient();
    const response = await client.patch(endpoint, data);
    return response.data;
}
export async function supabaseDelete(endpoint, data) {
    const client = getClient();
...
export function handleSupabaseError(error) {
    if (axios.isAxiosError(error)) {
        ...
        if (axErr.response) {
            ...
            switch (status) {
                case 401:
                    return ('Error: Supabase authentication failed. Your SUPABASE_ACCESS_TOKEN may be invalid or expired. ' +
