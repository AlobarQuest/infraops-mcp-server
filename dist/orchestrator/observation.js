 ...
 */
export const INSTANCE_SUBJECTS = {
...
function statusFor(facts) {
    if (facts.instance_ok !== true)
        return 'unknown';
    return count(facts.total_proposals) > 0 ? 'degraded' : 'passed';
}
function severityFor(facts) {
    if (facts.instance_ok !== true)
        return 'warning';
    const byRisk = (facts.by_risk ?? {});
    if (count(byRisk.destructive) > 0)
        return 'critical';
    return count(facts.total_proposals) > 0 ? 'warning' : 'info';
}
function summaryFor(subject, facts) {
    if (facts.instance_ok !== true)
        return `${subject} — instance unreachable`;
...
export function buildObservation(report, instance) {
