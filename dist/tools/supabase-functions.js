 ...
 */
import { z } from 'zod';
import { supabaseGet, supabasePost, supabasePatch, supabaseDelete, handleSupabaseError, } from '../services/supabase-client.js';
export function registerSupabaseFunctionTools(server) {
    // ── List Functions ────────────────────────────────────────────────
    server.registerTool('supabase_list_functions', {
        title: 'List Supabase Edge Functions',
        description: 'List all Edge Functions deployed to a Supabase project. Returns function slugs, names, and deployment status.',
        inputSchema: {
        ...
        try {
            const result = await supabaseGet(`/projects/${ref}/functions`);
            return {
    ...
    // ── Get Function ──────────────────────────────────────────────────
    server.registerTool('supabase_get_function', {
        title: 'Get Supabase Edge Function',
        description: 'Get details for a specific Edge Function deployed to a Supabase project — name, slug, verify_jwt setting, and deployment info.',
        inputSchema: {
            ref: z.string().describe("Supabase project reference (e.g. 'abcdefghijklmnop')"),
        ...
        try {
            const result = await supabaseGet(`/projects/${ref}/functions/${slug}`);
            return {
                content: [{ type: 'text', text: JSON.stringify(result, null, 2) }],
            };
        }
    ...
    // ── Create Function ───────────────────────────────────────────────
    server.registerTool('supabase_create_function', {
        title: 'Create Supabase Edge Function',
        description: 'Deploy a new Edge Function to a Supabase project. Provide the function name, slug, and optionally the source code body.',
        inputSchema: {
            ref: z.string().describe("Supabase project reference (e.g. 'abcdefghijklmnop')"),
        ...
        try {
            ...
            const result = await supabasePost(`/projects/${params.ref}/functions`, requestBody);
            return {
                content: [{ type: 'text', text: JSON.stringify(result, null, 2) }],
            };
        }
    ...
    // ── Update Function ───────────────────────────────────────────────
    server.registerTool('supabase_update_function', {
        title: 'Update Supabase Edge Function',
        description: 'Update an existing Edge Function on a Supabase project. Can update the name, JWT verification setting, or source code.',
        inputSchema: {
            ref: z.string().describe("Supabase project reference (e.g. 'abcdefghijklmnop')"),
        ...
        try {
            ...
            if (params.verify_jwt !== undefined)
                requestBody.verify_jwt = params.verify_jwt;
            if (params.body !== undefined)
                requestBody.body = params.body;
            const result = await supabasePatch(`/projects/${params.ref}/functions/${params.slug}`, requestBody);
            return {
                content: [{ type: 'text', text: JSON.stringify(result, null, 2) }],
            };
        }
    ...
    // ── Delete Function ───────────────────────────────────────────────
    server.registerTool('supabase_delete_function', {
        title: 'Delete Supabase Edge Function',
        description: 'WARNING: Permanently deletes an Edge Function from a Supabase project. This action is irreversible.',
        inputSchema: {
            ref: z.string().describe("Supabase project reference (e.g. 'abcdefghijklmnop')"),
        ...
        try {
            const result = await supabaseDelete(`/projects/${ref}/functions/${slug}`);
            return {
                content: [
                    {
                        type: 'text',
...
//# sourceMappingURL=supabase-functions.js.map