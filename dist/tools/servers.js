...
export function registerServerTools(server) {
    ...
    server.registerTool("coolify_list_servers", {
        ...
        try {
            const servers = await coolifyGet("/servers");
            return {
                content: [
    ...
    server.registerTool("coolify_get_server", {
        ...
        try {
            const srv = await coolifyGet(`/servers/${uuid}`);
            return {
    ...
    server.registerTool("coolify_validate_server", {
        ...
        try {
            const result = await coolifyGet(`/servers/${uuid}/validate`);
            return {
    ...
    server.registerTool("coolify_server_resources", {
        ...
        try {
            const resources = await coolifyGet(`/servers/${uuid}/resources`);
            return {
                content: [
    ...
    server.registerTool("coolify_server_domains", {
        ...
        try {
            const domains = await coolifyGet(`/servers/${uuid}/domains`);
            return {
                content: [
