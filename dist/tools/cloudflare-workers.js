...
export function registerCloudflareWorkersTools(server) {
    ...
    server.registerTool("cloudflare_delete_worker", {
        ...
        try {
            const accountId = getAccountId();
            const result = await cloudflareDelete(`/accounts/${accountId}/workers/scripts/${script_name}`);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.registerTool("cloudflare_delete_kv_namespace", {
        ...
        try {
            const accountId = getAccountId();
            const result = await cloudflareDelete(`/accounts/${accountId}/storage/kv/namespaces/${namespace_id}`);
            return {
                content: [
                    {
                        type: "text",
    ...
    }, async ({ namespace_id, prefix, limit, }) => {
        try {
            const accountId = getAccountId();
            const params = {};
            if (prefix)
    ...
    server.registerTool("cloudflare_get_kv_value", {
        ...
        try {
            ...
            const result = await cloudflareGet(`/accounts/${accountId}/storage/kv/namespaces/${namespace_id}/values/${key_name}`);
            return {
                content: [
                    {
                        type: "text",
