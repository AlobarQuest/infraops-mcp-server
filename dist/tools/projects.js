...
export function registerProjectTools(server) {
    ...
    server.registerTool('coolify_create_project', {
        ...
        description: 'Create a new project. After creation, add environments to it, then deploy applications/databases/services into those environments.',
        inputSchema: {
    ...
    // ── Update Project ───────────────────────────────────────────────
    server.registerTool('coolify_update_project', {
    ...
    // ── List Environments ────────────────────────────────────────────
    server.registerTool('coolify_list_environments', {
        title: 'List Project Environments',
        description: 'List all environments within a project. Environments separate resources (e.g. production vs staging).',
        inputSchema: {
            project_uuid: z.string().min(1).describe('UUID of the parent project'),
            instance: CoolifyInstanceSchema,
        },
        annotations: {
            readOnlyHint: true,
    ...
    server.registerTool('coolify_create_environment', {
        ...
        description: "Create a new environment within a project (e.g. 'production', 'staging', 'preview').",
        inputSchema: {
            project_uuid: z.string().min(1).describe('UUID of the parent project'),
            name: z.string().min(1).describe('Environment name (e.g. production, staging)'),
            instance: CoolifyInstanceRequiredSchema,
        },
        annotations: {
            readOnlyHint: false,
    ...
    // ── Delete Environment ───────────────────────────────────────────
    server.registerTool('coolify_delete_environment', {
        title: 'Delete Project Environment',
        description: 'Delete an environment from a project. WARNING: All resources in this environment will be removed.',
        inputSchema: {
            project_uuid: z.string().min(1).describe('UUID of the parent project'),
            environment_name: z.string().min(1).describe('Name of the environment to delete'),
            instance: CoolifyInstanceRequiredSchema,
        },
        annotations: {
            readOnlyHint: false,
