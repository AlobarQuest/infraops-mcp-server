 ...
 */
import { z } from "zod";
import { namecheapCommand, handleNamecheapError, getNamecheapEnvironment, } from "../services/namecheap-client.js";
export function registerNamecheapDomainTools(server) {
    // ═══════════════════════════════════════════════════════════════════
    //  ENVIRONMENT INFO
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_domains_get_env", {
        title: "Namecheap Environment",
        description: "Show which Namecheap API environment is active (sandbox or production) and the configured API user.",
        inputSchema: {},
    ...
    //  LIST DOMAINS
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_domains_list", {
        title: "List Domains",
        ...
        try {
            ...
            if (params.searchTerm)
                commandParams.SearchTerm = params.searchTerm;
    ...
    //  GET DOMAIN INFO
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_domains_get_info", {
        title: "Get Domain Info",
        description: "Get detailed information about a specific domain: status, creation/expiry dates, " +
    ...
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_domains_check", {
    ...
    //  REGISTER DOMAIN
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_domains_register", {
        title: "Register Domain",
        description: "Register a new domain. Requires registrant contact information. " +
        ...
        try {
            ...
            if (params.nameservers)
                commandParams.Nameservers = String(params.nameservers);
    ...
    //  RENEW DOMAIN
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_domains_renew", {
        title: "Renew Domain",
        description: "Renew a domain registration. ⚠️ This is a PAID operation — charges your account balance.",
        inputSchema: {
    ...
    //  REACTIVATE DOMAIN
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_domains_reactivate", {
        title: "Reactivate Domain",
        description: "Reactivate an expired domain (if still within the redemption grace period). " +
    ...
    //  GET REGISTRAR LOCK
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_domains_get_lock", {
    ...
    //  SET REGISTRAR LOCK
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_domains_set_lock", {
    ...
    //  GET CONTACTS
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_domains_get_contacts", {
    ...
    //  GET TLD LIST
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_domains_get_tld_list", {
        title: "Get TLD List",
...
//# sourceMappingURL=namecheap-domains.js.map