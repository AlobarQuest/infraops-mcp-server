 ...
 */
import { z } from "zod";
import { sshExec, sshReadFile, sshWriteFile, handleSSHError, } from "../services/ssh-client.js";
export function registerVPSTools(server) {
    // ── Execute Command ──────────────────────────────────────────────
    server.registerTool("vps_exec", {
        title: "Execute VPS Command",
        description: "Run a shell command on the VPS via SSH. Returns stdout, stderr, and exit code. " +
    ...
    server.registerTool("vps_health", {
        ...
        description: "Get a comprehensive health snapshot of the VPS: uptime, CPU load, memory usage, " +
    ...
    server.registerTool("vps_write_file", {
        ...
        description: "Write content to a file on the VPS. Creates the file if it doesn't exist, overwrites if it does.",
        inputSchema: {
    ...
    server.registerTool("vps_docker_ps", {
        ...
        description: "List running Docker containers on the VPS. Shows name, image, status, ports, and resource usage.",
        inputSchema: {
            all: z
                .boolean()
                .default(false)
        ...
        try {
            const flag = all ? " -a" : "";
            const result = await sshExec(`docker ps${flag} --format 'table {{.Names}}\\t{{.Image}}\\t{{.Status}}\\t{{.Ports}}\\t{{.Size}}'`, { timeout: 15000 });
            return {
                content: [{ type: "text", text: result.stdout }],
            };
        }
    ...
    server.registerTool("vps_docker_logs", {
        ...
        description: "Retrieve logs from a Docker container on the VPS. Useful for debugging app issues directly.",
        inputSchema: {
            container: z
                .string()
                .min(1)
    ...
    server.registerTool("vps_docker_stats", {
        ...
        description: "Get current CPU, memory, network, and disk I/O usage for all running containers. " +
        ...
        try {
            const result = await sshExec("docker stats --no-stream --format 'table {{.Name}}\\t{{.CPUPerc}}\\t{{.MemUsage}}\\t{{.NetIO}}\\t{{.BlockIO}}'", { timeout: 15000 });
            return {
                content: [{ type: "text", text: result.stdout }],
            };
        }
