 ...
 */
import { z } from "zod";
import { namecheapCommand, handleNamecheapError, splitDomain, } from "../services/namecheap-client.js";
// ── Helpers ──────────────────────────────────────────────────────────
const RECORD_TYPES = ["A", "AAAA", "CNAME", "MX", "MXE", "TXT", "NS", "URL", "URL301", "FRAME"];
/**
 * Fetch current host records for a domain.
 */
async function fetchHosts(domain) {
    const { sld, tld } = splitDomain(domain);
    const result = await namecheapCommand("namecheap.domains.dns.getHosts", {
        SLD: sld,
        TLD: tld,
    });
    const hostResult = result.CommandResponse?.DomainDNSGetHostsResult;
    if (!hostResult)
        return [];
    const rawHosts = hostResult.host;
    if (!rawHosts)
        return [];
    const hostArray = Array.isArray(rawHosts) ? rawHosts : [rawHosts];
    return hostArray.map((h) => ({
 ...
 */
async function pushHosts(domain, hosts) {
    const { sld, tld } = splitDomain(domain);
    const params = {
        SLD: sld,
        TLD: tld,
...
// ── Tool Registration ────────────────────────────────────────────────
export function registerNamecheapDNSTools(server) {
    // ═══════════════════════════════════════════════════════════════════
    //  GET HOSTS — List all DNS records
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_dns_get_hosts", {
        title: "Get DNS Records",
        description: "List all DNS host records for a domain. Returns record name, type, address, TTL, and MX preference.",
        inputSchema: {
    ...
    //  GET NAMESERVERS
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_dns_get_servers", {
        title: "Get Nameservers",
        description: "Get the nameservers configured for a domain. Shows whether it uses Namecheap default DNS or custom nameservers.",
        inputSchema: {
            domain: z.string().min(3).describe("Domain name (e.g. 'devonwatkins.com')"),
        },
        annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    ...
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_dns_add_record", {
        title: "Add DNS Record",
        description: "Add a single DNS record to a domain while preserving all existing records. " +
    ...
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_dns_update_record", {
        title: "Update DNS Record",
        description: "Update an existing DNS record matched by name and type. " +
        ...
        try {
            ...
            const idx = hosts.findIndex((h) => h.Name.toLowerCase() === params.name.toLowerCase() && h.Type.toUpperCase() === params.type);
            if (idx === -1) {
                return {
                    isError: true,
                    content: [{
            ...
            if (params.newAddress !== undefined)
                hosts[idx].Address = params.newAddress;
            if (params.newTtl !== undefined)
                hosts[idx].TTL = params.newTtl;
            if (params.newMxPref !== undefined)
                hosts[idx].MXPref = params.newMxPref;
    ...
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_dns_delete_record", {
        title: "Delete DNS Record",
        description: "Delete a DNS record matched by name, type, and optionally address. " +
    ...
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_dns_set_hosts", {
        title: "Set All DNS Records (Replace)",
        description: "ADVANCED: Replace ALL DNS records for a domain with the provided set. " +
        ...
        try {
            const hosts = params.records.map((r) => ({
                Name: r.name,
                Type: r.type,
                Address: r.address,
                MXPref: r.mxPref,
    ...
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_dns_set_nameservers", {
        title: "Set Custom Nameservers",
        description: "Set custom nameservers for a domain (e.g. to use Cloudflare or another DNS provider). " +
    ...
    //  SET DEFAULT DNS
    // ═══════════════════════════════════════════════════════════════════
    server.registerTool("namecheap_dns_set_default", {
...
//# sourceMappingURL=namecheap-dns.js.map