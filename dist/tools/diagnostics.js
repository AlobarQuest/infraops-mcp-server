...
async function resolveAppUuid(query, instance) {
    if (UUID_RE.test(query))
        return query;
    const apps = await coolifyGet("/applications", undefined, instance);
    const q = query.toLowerCase();
    const matches = apps.filter((a) => a.name?.toLowerCase().includes(q) || a.fqdn?.toLowerCase().includes(q));
    if (matches.length === 0)
...
async function resolveServerUuid(query, instance) {
    if (UUID_RE.test(query))
        return query;
    const servers = await coolifyGet("/servers", undefined, instance);
    const q = query.toLowerCase();
    const matches = servers.filter((s) => s.name?.toLowerCase().includes(q) || s.ip?.toLowerCase().includes(q));
    if (matches.length === 0)
...
export function registerDiagnosticTools(server) {
    // ── diagnose_app ─────────────────────────────────────────────────────
    server.registerTool("coolify_diagnose_app", {
        title: "Diagnose Application",
        description: "Composite diagnostic for an application: fans out to fetch details, recent logs, " +
    ...
    }, async ({ query, log_lines, instance }) => {
        let uuid;
        try {
        ...
        const [appResult, logsResult, envsResult, deploymentsResult] = await Promise.allSettled([
            coolifyGet(`/applications/${uuid}`, undefined, instance),
            coolifyGet(`/applications/${uuid}/logs`, { lines: log_lines }, instance),
            coolifyGet(`/applications/${uuid}/envs`, undefined, instance),
            coolifyGet(`/applications/${uuid}/deployments`, undefined, instance),
        ]);
        const errors = [];
    ...
    // ── diagnose_server ───────────────────────────────────────────────────
    server.registerTool("coolify_diagnose_server", {
        title: "Diagnose Server",
        description: "Composite diagnostic for a server: fetches details, resources, domains, and " +
        ...
        const [serverResult, resourcesResult, domainsResult, validationResult] = await Promise.allSettled([
            coolifyGet(`/servers/${uuid}`, undefined, instance),
            coolifyGet(`/servers/${uuid}/resources`, undefined, instance),
            coolifyGet(`/servers/${uuid}/domains`, undefined, instance),
            coolifyGet(`/servers/${uuid}/validate`, undefined, instance),
        ]);
        const errors = [];
        function extract(result, label) {
            if (result.status === "fulfilled")
        ...
        const unhealthy = (resources ?? []).filter((r) => r.status.includes("exited") || r.status.includes("unhealthy") || r.status.includes("error"));
        if (unhealthy.length > 0)
            issues.push(`${unhealthy.length} unhealthy resource(s)`);
        const output = {
            server: server
                ? {
                    uuid: server["uuid"],
    ...
    // ── find_issues ───────────────────────────────────────────────────────
    server.registerTool("coolify_find_issues", {
        title: "Find Infrastructure Issues",
        description: "Scan all servers, applications, databases, and services for unhealthy or " +
        ...
        const issues = [];
        const unhealthyStatus = (s) => s.includes("exited") || s.includes("unhealthy") || s.includes("error") || s === "stopped";
