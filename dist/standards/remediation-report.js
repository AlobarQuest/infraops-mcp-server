export function buildRemediationReport(args) {
    const count = (s) => args.applied.filter((a) => a.status === s).length;
    return {
        schema_version: 1,
...
export function renderRemediationMarkdown(r) {
    const t = r.totals;
    const lines = [];
    lines.push(`# Infra Remediation — ${r.generated_at}`);
    lines.push("");
    lines.push(`**${t.applied} fixed**, ${t.escalated} need attention ` +
        `(skipped ${t.skipped}, failed ${t.failed}, self-resolved ${t.self_resolved}).`);
    if (t.runaway_tripped) {
        lines.push("");
        lines.push(`> ⚠️ **Safety guard tripped:** the live safe-fix count exceeded MAX_AUTO_APPLIES, ` +
...
//# sourceMappingURL=remediation-report.js.map