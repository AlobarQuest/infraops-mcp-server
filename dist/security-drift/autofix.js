...
function currentUid(opts) {
    if (opts.getUid)
        return opts.getUid();
    return typeof process.getuid === 'function' ? process.getuid() : null;
}
function appendRollback(rollbackLog, rec) {
...
export function autoFix(target, opts) {
    ...
    try {
        ...
        if (e?.code === 'ELOOP')
            return { status: 'blocked', target, reason: 'target is a symlink (O_NOFOLLOW)' };
        return {
            status: 'blocked',
            target,
            reason: `open failed: ${e?.code ?? e?.message ?? 'unknown'}`,
        };
    }
    try {
        const st = fs.fstatSync(fd);
        if (!st.isFile())
            return { status: 'blocked', target, reason: 'not a regular file' };
        if (st.nlink !== 1)
            return { status: 'blocked', target, reason: `hardlinked (nlink=${st.nlink})` };
        const uid = currentUid(opts);
        if (uid !== null && st.uid !== uid)
            return { status: 'blocked', target, reason: `owner uid ${st.uid} != ${uid}` };
        const parentMode = fs.lstatSync(path.dirname(target)).mode & 0o777;
        if ((parentMode & 0o002) !== 0)
            return {
                status: 'blocked',
                target,
                reason: `parent dir other-writable (${parentMode.toString(8)})`,
            };
        const priorMode = st.mode & 0o777;
        if (priorMode === 0o600)
            return { status: 'blocked', target, reason: 'already 600 (nothing to fix)' };
        ...
        return {
            status: 'blocked',
            target,
            reason: `guard/chmod failed: ${e?.code ?? e?.message ?? 'unknown'}`,
        };
    }
    finally {
        fs.closeSync(fd);
...
export function runAutoFixes(targets, opts) {
    const run = { applied: [], blocked: [] };
    for (const target of targets) {
        ...
        if (r.status === 'applied')
            run.applied.push({ target: r.target, priorMode: r.priorMode });
        else
            run.blocked.push({ target: r.target, reason: r.reason });
    }
    return run;
}
