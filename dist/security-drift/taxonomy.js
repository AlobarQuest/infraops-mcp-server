...
export function isFalsePositive(f, opts) {
    if (FP_PATH_PATTERNS.some((re) => re.test(f.target)))
        return true;
    if (opts.fpExtra?.some((s) => f.target.includes(s) || f.detail.includes(s)))
        return true;
    return false;
}
