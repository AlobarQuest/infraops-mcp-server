...
# Deploy the detector + skills linter to ~/.claude/bin (overwrites with the repo copy,
# which is the source of truth). No secrets are involved — these are read-only scanners.
install -m 755 "$REPO/scripts/security-scan.sh" "$BIN_DIR/security-scan.sh"
install -m 755 "$REPO/scripts/skills-security-scan.sh" "$BIN_DIR/skills-security-scan.sh"
